...
void validStrLen(const long unsigned maxsz, const char *src1, const char *src2) {
    if (strlen(src1) + strlen(src2) >= maxsz) {
        fprintf(stderr, "The input command is too long: %ld/%ld\n", strlen(src1) + strlen(src2), maxsz);
        _exit(0);
    }
}
    ...
    switch (cmd->type) {
        ...
        case ' ':
            ...
            if (!access(ecmd->argv[0], F_OK)) {  // 若指令所指文件存在当前文件夹，则执行本地文件
                ...
                char path[100];
                validStrLen(sizeof(path), rootusr, ecmd->argv[0]);
                strcpy(path, root);
...
int main(void) {
    ...
    while (getcmd(buf, sizeof(buf)) >= 0) {
        ...
        // 生成一个shell进程的副本，父shell调用wait，而子进程执行用户命令。
